    ...
    async def detect(
        ...
        if not os.path.exists(index_path):
            return DetectorResult(
                ...
                confidence=0.3,
                signals=[
                    "No style index exists for this repository yet. "
                    "Using neutral score."
                ]
            )
        ...
        # In IndexFlatIP, distance is inner product
        # (cosine similarity since vectors are normalized)
        # avg_similarity = np.mean(distances[0])
        # Distance > 0.4 (low similarity) -> foreign content
        # -> high AI probability
        ...
        return DetectorResult(
            ...
            signals=[
                ...
                (
                    "Content is stylistically foreign to this repo's "
                    "writing history"
                    if avg_sim < 0.2
                    else "Content matches repo writing style"
                )
            ]
        )
