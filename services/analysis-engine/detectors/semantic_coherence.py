    ...
    async def detect(
        ...
        if len(parts) < 2:
            return DetectorResult(
                ...
                confidence=0.5,
                signals=[
                    "Content format not suitable for semantic coherence check."
                ]
            )
        ...
        # Logic: AI PRs sometimes have a generic title but weird code,
        # or vice versa
        ...
        if similarity < 0.3:
            ...
            signals = [
                (
                    "Critical semantic mismatch between PR description "
                    f"and code changes (Sim: {similarity:.2f})"
                )
            ]
        elif similarity < 0.45:
            score = 0.6
            signals = [
                f"Low semantic coherence detected ({similarity:.2f})"
            ]
        else:
            score = 0.0
            signals = [
                (
                    "PR description semantically aligns with code changes "
                    f"({similarity:.2f})"
                )
            ]
