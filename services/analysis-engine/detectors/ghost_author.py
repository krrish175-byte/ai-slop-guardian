    ...
    async def detect(
        ...
        if not history or len(history) < 3:
            return DetectorResult(
                ...
                confidence=0.3,
                signals=[
                    "Insufficient history for ghost author detection "
                    "(need >= 3 PRs)."
                ]
            )
        ...
        if deviation > 0.5:
            ...
            signals = [
                (
                    "Significant style deviation from 10 historical PRs: "
                    f"{deviation * 100:.1f}%"
                )
            ]
        elif deviation > 0.3:
            ...
            signals = [
                (
                    "Moderate style deviation from history: "
                    f"{deviation * 100:.1f}%"
                )
            ]
        else:
            ...
            signals = [
                (
                    "Stylistic match with historical contributions "
